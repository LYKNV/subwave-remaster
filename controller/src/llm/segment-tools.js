...
export function buildSegmentTools(ctx, state, caps) {
    ...
    tools.getHeadlines = tool({
      ...
      execute: async () => {
        try {
          ...
          const fresh = items.filter(it => !state.seenHeadlines.has(hashHeadline(it.title)));
          // Mark surfaced headlines as seen so a later tick doesn't re-offer
          // them — same "burn on read" approach as the old news skill.
          for (const it of fresh.slice(0, 6)) state.seenHeadlines.add(hashHeadline(it.title));
  ...
  if (kinds.has('web-search') && config.search.apiKey) {
    tools.searchArtistNews = tool({
      ...
      execute: async () => {
        ...
        try {
          const data = await tavilySearch(`${artist} musician latest news`);
