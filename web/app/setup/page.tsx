...
// /docs/setup for the deeper documentation.
//
// (Force-dynamic — checks the controller's /setup/status on every request to
// avoid serving a stale "needs setup" view after the operator finishes.)

import { useEffect, useState } from 'react';
import Link from 'next/link';
import WizardShell from '@/components/setup/wizard/WizardShell';
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-12">
        ...
        <p className="mt-2 text-sm text-ink/70">
          Open <code>{API_URL}/setup/status</code> in another tab to confirm it&apos;s up. Error: <code>{error}</code>
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <p className="mt-2 text-sm text-ink/70">
        ...
        {status.setupCompletedAt
          ...
          : '.'}{' '}
        Manage everything from the admin panel; read <Link href="/docs/setup" className="bs-link">the setup docs</Link> if you want
